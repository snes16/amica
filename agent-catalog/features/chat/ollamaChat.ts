...
export async function getOllamaChatResponseStream(config: ChatbotBackend["ollama"],messages: Message[]) {
...
export async function getOllamaVisionChatResponse(config: VisionBackend["vision_ollama"],messages: Message[], imageData: string) {
  const headers: Record<string, string> = {
    "Content-Type": "application/json",
  };
