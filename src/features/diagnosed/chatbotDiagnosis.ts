import { ChatbotBackend } from "@/utils/diagnosisUtils";
...
async function safeFetch(fullUrl: string, options: RequestInit): Promise<string> {
  try {
...
const backendHandlers: Record<
  string,
  (params: ChatbotBackend) => Promise<string>
> = {
  openai: async (params) => {
